...
{	_entry:;
	...
	switch (transfer->status) {
		case LIBUSB_TRANSFER_COMPLETED:
			...
			g_atomic_int_set(&ctx->stats->interval_ms, (guint)(1e-6 * (ctx->buffer->priv->timestamp_ns - prev)));
...
{	_entry:;
	...
	switch (ctx->buffer->priv->status) {
		case ARV_BUFFER_STATUS_FILLING:
			...
			ctx->stats->n_completed_buffers += 1;
			break;
		default:
						...
						if (offset != buffer->priv->size) {
							...
							if (thread_data->callback != NULL)
								thread_data->callback (thread_data->callback_data,
										       ARV_STREAM_CALLBACK_TYPE_BUFFER_DONE,
										       buffer);
							thread_data->stats.n_completed_buffers++;
							buffer = NULL;
						}
					}
					break;
...
{	_entry:;
	...
	thread_data->stats.n_completed_buffers = 0;
	thread_data->stats.n_failures = 0;
	thread_data->stats.n_underruns = 0;
...
{	_entry:;
	...
	*n_completed_buffers = thread_data->stats.n_completed_buffers;
	*n_failures = thread_data->stats.n_failures;
	*n_underruns = thread_data->stats.n_underruns;
}
